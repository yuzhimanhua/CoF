...
PlCheckpointState = collections.namedtuple(
    "PlCheckpointState",
    [
        'epoch',
        'global_step',
...
def setup_for_distributed_mode(
    ...
    model.to(device)
    # TODO(chenghao): Disables this for now.
    # if fp16:
    #     try:
    #         import apex
...
def get_model_file(args, file_prefix) -> str:
    ...
    if args.model_file is not None and model_file is None:
        raise ValueError(f"Can not read in the specified model_file {args.model_file}")
    return model_file


def load_states_from_checkpoint(model_file: str, from_pl: bool = False) -> CheckpointState:
    ...
    if from_pl:
        state_dict.pop('pytorch-lightning_version')
        return PlCheckpointState(**state_dict)
