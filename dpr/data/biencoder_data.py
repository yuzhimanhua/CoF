...
from torch import Tensor as T, norm

from dpr.data.tables import Table
from dpr.utils.data_utils import read_data_from_json_files, Tensorizer, read_data_from_jsonl_files

logger = logging.getLogger(__name__)
# BiEncoderPassage = collections.namedtuple("BiEncoderPassage", ["text", "title", "psg_id"])
...
class BiEncoderUniSample:
    ...
    negative_passages: List[BiEncoderPassage]
    hard_negative_passages: List[BiEncoderPassage]
    hop1_passages: List[BiEncoderPassage]
    query_rep_start: int
    query_rep_end: int
...
class JsonQADataset(Dataset):
    def __init__(
        ...
        self.ctx_instruct = ctx_instruct
        logger.info("Data file: %s", self.file)
        logger.info("data type: %s" % data_type)
        logger.info("Data files: %s", self.data_files)
        if query_instruct:
            logger.info("Data has query instruction: %s" % self.query_instruct)

        if ctx_instruct:
            logger.info("Data has ctx instruction: %s" % self.ctx_instruct)
    ...
    def __getitem__(self, index) -> BiEncoderUniSample:
        ...
        # print(self.query_instruct)
        return BiEncoderUniSample(
            data_type=self.data_type,
            query=self._process_query(json_sample["question"]),
            query_instruct=self.query_instruct,
            ctx_instruct=self.ctx_instruct,
            query_rep_start=0,
            query_rep_end=0,
...
class JsonlQADataset(Dataset):
    ...
    def load_data(self):
        self.data_files = get_dpr_files(self.file)
        data = read_data_from_jsonl_files(self.data_files)
        # filter those without positive ctx
        self.data = [r for r in data if len(r["positive_ctxs"]) > 0]
        logger.info("Total cleaned data size: {}".format(len(self.data)))
...
class JsonlSpanQADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
        ...
        logger.info("data type: %s" % data_type)
        if query_instruct:
            logger.info("Data has query instruction: %s" % query_instruct)
        if ctx_instruct:
            logger.info("Data has ctx instruction: %s" % ctx_instruct)
    ...
    def __getitem__(self, index) -> BiEncoderUniSample:
        ...
        hop1_ctxs = json_sample.get("hop1_ctxs", [])
        ...
        return BiEncoderUniSample(
            ...
            query_instruct=self.query_instruct,
            ctx_instruct=self.ctx_instruct,
            query_rep_start=int(json_sample["ent_char_start"]),
            query_rep_end=int(json_sample["ent_char_end"]),
            positive_passages=[create_passage(ctx) for ctx in positive_ctxs],
            negative_passages=[create_passage(ctx) for ctx in negative_ctxs],
            hard_negative_passages=[create_passage(ctx) for ctx in hard_negative_ctxs],
            hop1_passages=[create_passage(ctx) for ctx in hop1_ctxs],
        )
