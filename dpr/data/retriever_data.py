...
EntityQASample = collections.namedtuple("EntityQuerySample", ["query", "q_start", "q_end", "id", "answers"])
...
class JsonListQASrc(QASrc):
    ...
    def load_data(self):
        super().load_data()
        data = []
        with open(self.file, mode="r") as json_fin:
            jsonl_reader = json.load(json_fin)
...
class JsonlEntityQASrc(QASrc):
    def __init__(
        ...
        answers_attr: str = "answers",
        id_attr: str = "id",
        q_start_idx_attr: str = "ent_char_start",
        q_end_idx_attr: str = "ent_char_end",
        special_query_token: str = None,
        query_special_suffix: str = None,
    ):
        super().__init__(file, selector, special_query_token, query_special_suffix)
        self.question_attr = question_attr
        self.answers_attr = answers_attr
        self.q_start_idx_attr = q_start_idx_attr
        self.q_end_idx_attr = q_end_idx_attr
        self.id_attr = id_attr
    ...
    def load_data(self):
        ...
        with jsonlines.open(self.file, mode="r") as jsonl_reader:
            for jline in jsonl_reader:
                question = jline[self.question_attr]
                answers = jline[self.answers_attr] if self.answers_attr in jline else []
                q_start_idx = jline[self.q_start_idx_attr]
                q_end_idx = jline[self.q_end_idx_attr]
                id = None
                if self.id_attr in jline:
                    id = jline[self.id_attr]
                data.append(EntityQASample(question, q_start_idx, q_end_idx, id, answers))
        self.data = data
...
class JsonlEntitySrc(RetrieverData):
    def __init__(
        self,
        file: str,
        id_col: str = "idx",
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        super().load_data()
        with open(self.file) as ifile:
            reader = [json.loads(line) for line in ifile]
            for row in reader:
                if self.id_prefix:
                    sample_id = self.id_prefix + str(row[self.id_col])
                else:
...
class MyJsonlTablesCtxSrc(object):
    ...
    def load_data_to(self, ctxs: Dict):
        ...
        for chunk in table_chunks:
            sample_id = self.id_prefix + '_' + chunk['chunk_id']
