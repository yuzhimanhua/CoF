...
class BiEncoderBatch:
    ...
    hard_negatives: List
    encoder_type: str
    query_instruct_ids: T = None
    query_instruct_masks: T = None
    ctx_instruct_ids: T = None
    ctx_instruct_masks: T = None
    question_attn_mask: T = None
    question_rep_pos: T = None
    ctx_attn_mask: T = None
    ctx_rep_pos: T = None
    pid_tensors: T = None
    full_positive_pids: List = None
    query_expert_ids: T = None
    ctx_expert_ids: T = None
    flip_query_expert_ids: T = None
    flip_ctx_expert_ids: T = None
    neg_span_pos: T = None
...
@dataclass
class InstructBiEncoderBatch:
    question_ids: T
    question_segments: T
    query_instruct_ids: T
    query_instruct_masks: T
    context_ids: T
    ctx_segments: T
    ctx_instruct_ids: T
    ctx_instruct_masks: T
    is_positive: List
    hard_negatives: List
    encoder_type: str
    question_attn_mask: T = None
...
class InstructBiEncoderUni(nn.Module):
    ...
    def __init__(
        ...
        fix_q_encoder: bool = False,
        fix_ctx_encoder: bool = False,
        fix_instruct_encoder: bool = False,
        q_rep_method: str = "start_end_concat",
        ctx_rep_method: str = "cls",
        entity_drop_prob: float = 0.3,
        title_drop_prob: float = 0.1,
        deep_instruct_fusion: bool = True,
    ):
        super(InstructBiEncoderUni, self).__init__()
        self.question_model = question_model
        self.ctx_model = ctx_model
        self.instruct_model = instruct_model
        self.fix_q_encoder = fix_q_encoder
        self.fix_ctx_encoder = fix_ctx_encoder
        self.fix_instruct_encoder = fix_instruct_encoder
        self.deep_instruct_fusion = deep_instruct_fusion
    ...
    def load_state(self, saved_state: CheckpointState,
        ...
        has_instruct_weight = False
        for var_name in saved_state.model_dict:
            if "instruct" in var_name:
                has_instruct_weight = True
                break
    ...
    def forward(
        ...
        assert query_instruct_ids is not None
        
        q_instruct_states, ctx_instruct_states = None, None
        if query_instruct_ids is not None:
            q_instruct_outputs = self.get_representation(
                self.instruct_model,
                query_instruct_ids,
                None,
                query_instruct_masks,
                self.fix_instruct_encoder,
                output_hidden_states=True,
            )
            q_instruct_states = q_instruct_outputs[2]
            assert q_instruct_states is not None

        if ctx_instruct_ids is not None:
            ctx_instruct_outputs = self.get_representation(
                self.instruct_model,
                ctx_instruct_ids,
                None,
                ctx_instruct_masks,
            ...
            ctx_instruct_states = ctx_instruct_outputs[2]
            assert ctx_instruct_states is not None
        else:
            ctx_instruct_states = q_instruct_states
            ctx_instruct_masks = query_instruct_masks
        ...
        q_outputs = self.get_representation(
            ...
            question_attn_mask,
            self.fix_q_encoder,
            instruct_hidden_states=q_instruct_states if self.deep_instruct_fusion else None,
            instruct_attention_mask=query_instruct_masks if self.deep_instruct_fusion else None,
        )
        ...
        if not self.deep_instruct_fusion and question_ids is not None:
            instruct_cls = q_instruct_outputs[1]
            bsz = q_pooled_out.size()[0]
            if instruct_cls.size()[0] == 1:
                instruct_cls = torch.tile(instruct_cls, (bsz, 1))
            q_pooled_out = self.query_instruct_proj(torch.concat([q_pooled_out, instruct_cls], dim=1))
        ...
        ctx_outputs= self.get_representation(
            ...
            instruct_hidden_states=ctx_instruct_states if self.deep_instruct_fusion else None,
            instruct_attention_mask=ctx_instruct_masks if self.deep_instruct_fusion else None,
        )
        ctx_pooled_out = ctx_outputs[1]
    ...
    def create_biencoder_uni_input(
        ...
        def process_single_ctx(ctx):
            if insert_title and ctx.title and random.random() > title_drop_prob:
                # TODO: This can be problematic for models wo/ [SEP] token.
                return " ".join([
        ...
        for sid, sample in enumerate(samples):
            ...
            if use_instruct and hasattr(sample, "query_instruct") and sid == 0:
                # Currently, all examples in a batch belongs to the same task.
                question_instruct = sample.query_instruct
                ctx_instruct = sample.ctx_instruct
        ...
        if use_instruct:
            ...
            query_instruct_ids = instruct_inputs.input_ids
            query_instruct_masks = instruct_inputs.attention_mask

            ctx_instruct_inputs =  tensorizer.encode_text(
                ctx_instruct,
                return_overflowing_tokens=False,
                return_tensors='pt',
            )
            ctx_instruct_ids = ctx_instruct_inputs.input_ids
            ctx_instruct_masks = ctx_instruct_inputs.attention_mask
        else:
            query_instruct_ids = None
            query_instruct_masks = None
            ctx_instruct_ids = None
            ctx_instruct_masks = None
        ...
        return BiEncoderBatch(
            ...
            query_instruct_ids=query_instruct_ids,
            query_instruct_masks=query_instruct_masks,
            context_ids=ctxs_tensor,
            ctx_segments=ctx_segments,
            ctx_attn_mask=ctx_attn_mask,
            ctx_rep_pos=None,
            ctx_instruct_ids=ctx_instruct_ids,
            ctx_instruct_masks=ctx_instruct_masks,
